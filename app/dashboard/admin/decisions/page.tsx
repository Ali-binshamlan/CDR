  ...
  const fetchDecisions = React.useCallback(async () => {
    ...
    try {
      ...
      if (status === 403) {
        ...
        console.error('Error fetching admin decisions:', err);
        const axiosErr = err as { response?: { data?: { error?: string } } };
        setError(axiosErr?.response?.data?.error || 'تعذّر جلب القرارات الموثّقة — تحقّق من الاتصال وأعد المحاولة.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#F4F7FB] flex flex-col items-center justify-center gap-4 text-center px-4" dir="rtl">
        <h2 className="text-xl font-black text-red-600">تعذّر تحميل القرارات</h2>
