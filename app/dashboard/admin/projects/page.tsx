...
export default function AdminProjectsPage() {
  ...
  // إلى أرقام صفرية أو حالات فارغة مضللة"): catch كان يميّز 403 فقط، أي
  // خطأ آخر يُبتلَع بصمت تامة (بلا حتى console.error) فتُعرض "لا توجد
  // مشاريع مطابقة".
  ...
  const fetchProjects = React.useCallback(async () => {
    ...
    try {
      ...
      if (status === 403) {
        ...
        setError(axiosErr?.response?.data?.error || 'تعذّر جلب المشاريع — تحقّق من الاتصال وأعد المحاولة.');
      }
    } finally {
      setIsLoading(false);
    }
