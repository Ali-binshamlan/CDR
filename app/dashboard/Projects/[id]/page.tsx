...
// حادثة إنتاج فعلية (2026-08-09): 3 ثوانٍ كانت تفترض أن كل تكرار GET
// "خفيف" بما يكفي لأن Compute الفعلي (خطة Free على Supabase) يتحمّله —
// فحص pg_stat_user_tables بعد ساعات قليلة فقط من التشغيل أظهر 2.5+ مليون
// استعلام على current_dust_decisions وحده (~220 استعلام/ثانية متواصلة)،
// بينما القرار الفعلي لا يتغيّر إلا مرة واحدة تقريباً كل دقيقة (معدّل
// provider-pull الحقيقي) — 19 من كل 20 طلب polling كانت تُعيد نفس القرار
// القديم بلا أي فائدة، وهذا التكرار الزائد كان المساهم الأكبر في استنزاف
// CPU (96% استخدام) الذي تسبَّب في تعليقات متكررة لكامل القاعدة. اشتراك
// Supabase Realtime أدناه (final_decisions، INSERT فوري) هو المسار الفعلي
// السريع فعلاً — يُحدّث الواجهة لحظة كتابة قرار جديد بلا انتظار أي فاصل،
// فتقليل الـpolling الاحتياطي هنا ليقارب معدّل الكتابة الفعلي (بدل تكراره
// عبثاً 20 ضعف تلك السرعة) لا يؤخر ظهور القرارات الجديدة عملياً — فقط يزيل
// الحمل الزائد بلا فائدة حقيقية.
const DASHBOARD_POLL_INTERVAL_MS = 20 * 1000;
