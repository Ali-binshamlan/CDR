...
export default function CreateProjectPage() {
  ...
  const complianceCheck = () => {
    ...
    if (projectForm.dmp_approval_status === 'APPROVED') {
      ...
      // طلب مستخدم صريح (بنود من دليل RCRC/NCEC لا وجود لها في أي مكان في
      // الكود سابقاً — لا كقاعدة آلية ولا كنص توعوي): ترشيد المياه (الباب
      // الخامس، 5.1)، بروتوكولات الطوارئ (5.2)، ومتطلبات خطة إدارة الغبار
      // الإجرائية (الباب الرابع) — نص توعوي بحت، لا يُغذّي أي قاعدة امتثال
      // ولا يمنع الحفظ، بنفس فلسفة بقية infoNotices أعلاه.
      infoNotices.push('ترشيد المياه: الاعتماد على مياه غير صالحة للشرب (جوفية مرخصة، صرف صحي معالج، أو رفض تحلية) بدل مياه الشرب في أعمال تثبيط الغبار، مع إعادة تدوير مياه غسيل الإطارات قدر الإمكان.');
      infoNotices.push('الاستعداد للطوارئ: إيقاف فوري للأنشطة المولّدة للغبار عند تعطل أنظمة الرذاذ/الرشاشات حتى استعادتها، مع توفر مصدر مياه احتياطي لضمان الترطيب حتى أثناء انقطاع الإمداد.');
      infoNotices.push('خطة إدارة الغبار (DMP): تُقدَّم للجهة التنظيمية قبل 30 يوماً تقويمياً على الأقل من بدء التجهيز، وتُحدَّث خلال 10 أيام عمل عند أي تغيير جوهري (نطاق العمل، المقاولون، ضوابط التحكم، مخطط الرصد)، مع تعيين منسّق تحكم في الغبار مسؤول عن التنفيذ، وتقديم تقرير إغلاق عند اكتمال الأعمال.');
    }
      ...
      <div className="w-full max-w-7xl bg-white border border-[#061B40]/10 rounded-2xl shadow-xl overflow-hidden grid grid-cols-1 lg:grid-cols-12">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 lg:col-span-8 border-l border-[#061B40]/10 h-[85vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-3 pt-3 border-t border-[#061B40]/5">
            ...
            {/* طلب مستخدم صريح — الباب الخامس، 5.3 من دليل RCRC/NCEC: ممارسة
                غير إلزامية صراحة في الدليل نفسه ("رغم أنها ليست إلزامية")، لذا
                تُعرض بتصميم مختلف تماماً عن صندوق الالتزامات أعلاه (لا رمادي
                محايد "التزام"، ولا أحمر/كهرماني "تحذير") لتفادي إيحاء الإلزام
                — بطاقة زرقاء فاتحة محايدة بعنوان "اقتراح" صراحة. لا تعتمد على
                dmp_approval_status (ممارسة اختيارية بمعزل عن اعتماد الخطة).
                لا تُغذّي أي قاعدة امتثال ولا تُخزَّن. */}
            <div className="rounded-lg p-3 space-y-1.5 border bg-sky-50 border-sky-200">
              <p className="text-[10px] font-black uppercase tracking-wide flex items-center gap-1.5 text-sky-600">
                <AlertTriangle className="w-3.5 h-3.5" />
                اقتراح غير إلزامي — ممارسة مبتكرة
              </p>
              <p className="text-[11px] font-bold pr-5 text-sky-700">
                💡 التثبيط الآلي: أنظمة رذاذ مرتبطة بأجهزة رصد الرياح أو الجسيمات العالقة (PM10) تُفعَّل تلقائياً عند الاقتراب من الحدود المسموح بها — ليست إلزامية، لكنها تُظهر ريادة وقد تصبح معياراً مستقبلاً.
              </p>
            </div>
          </div>
