...
describe('writeDeviceReading', () => {
  ...
  // اختبارات قبول صريحة (طلب المستخدم — تقرير المراجعة الخارجي: "وقت الحقل
  // القديم قد يصبح حديثاً بسبب حقل آخر... ومسار Pull لا يرفض timestamp
  // مستقبلياً مثل Push"): devices/ingest/route.ts كان يرفض observedAt
  // مستقبلياً (أكثر من دقيقتين تسامحاً) *قبل* استدعاء writeDeviceReading —
  // لكن provider-pull (عبر telemetry-worker → writeDeviceReading مباشرة) لم
  // يكن يخضع لهذا الفحص إطلاقاً. الفحص الآن هنا في writeDeviceReading نفسها،
  // فيغطي كلا المسارين بلا استثناء.
  describe('رفض observedAtIso المستقبلي (اختبار قبول صريح)', () => {
    it('observedAtIso أحدث من الآن بأكثر من دقيقتين → رفض صريح، لا كتابة', async () => {
      const { writeDeviceReading } = await import('./deviceReadingWriter');
      const futureIso = new Date(Date.now() + 3 * 60_000).toISOString();
      const result = await writeDeviceReading({
        deviceId: 'd1',
        projectId: 'p1',
        reading: { pm10: 30, observedAtIso: futureIso },
      });
      expect(result.success).toBe(false);
      if (!result.success) expect(result.error).toContain('المستقبل');
      expect(rpcCalls).toHaveLength(0);
    });

    it('observedAtIso أحدث من الآن بدقيقة واحدة فقط (ضمن هامش التسامح دقيقتين) → يُقبَل', async () => {
      const { writeDeviceReading } = await import('./deviceReadingWriter');
      const nearFutureIso = new Date(Date.now() + 60_000).toISOString();
      const result = await writeDeviceReading({
        deviceId: 'd1',
        projectId: 'p1',
        reading: { pm10: 30, observedAtIso: nearFutureIso },
      });
      expect(result.success).toBe(true);
      expect(rpcCalls).toHaveLength(1);
    });
