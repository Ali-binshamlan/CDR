...
export function computeSustainedPm10Status(
  ...
  // الموثَّق سابقاً هنا بتوحيد PM10 إلى 3 مستويات منفصلة تماماً بقطع سلسلة
  // 250 عند أي قراءة >340): ذلك التصميم كان يعني أن تجاوز 340 (تصعيد أعلى)
  // يُلغي منطقياً مدة التجاوز المتراكمة في [250,340] — قرار غير مبرَّر
  // تنظيمياً حسب مراجعة لاحقة: تصعيد الخطورة لا يجوز أن "يصفّر" استمرار
  // التعرض السابق، بل الاثنان يجب أن يتراكما معاً. الحسم الجديد:
  // sustainedMinutesAbove250/isSuspended250For30Min يصبحان استمراراً موحَّداً
  // فوق 250 بلا حد أعلى (upperExclusiveBound أُزيل) — أي قراءة ≥250، سواء
  // داخل [250,340] أو >340، تُسهم في نفس عداد الـ30 دقيقة. تأكيد مخالفة 340
  // (above340Streak/isConfirmedViolation340) يبقى عداداً منفصلاً تماماً
  // بمدته الخاصة (دقيقتان) للتوثيق التنظيمي — راجع pm10ThresholdRule في
  // rulebook.ts للأثر على القرار: تأكيد 340 الآن توثيق/تنبيه بلا إيقاف
  // فوري، والإيقاف الفعلي الوحيد لنشاط تجاوز 250 يبقى حصراً عند اكتمال
  // الـ30 دقيقة الموحَّدة.
  ...
  // بلا upperExclusiveBound الآن — استمرار ≥250 موحَّد عبر [250,340] و>340
  // معاً، راجع تعليق القرار التنظيمي أعلى هذه الدالة.
  ...
  // أولوية عرض الأدلة فقط (لا أولوية قرار — الإيقاف الفعلي الوحيد الآن هو
  // isSuspended250For30Min، راجع تعليق القرار التنظيمي أعلاه وpm10ThresholdRule
  // في rulebook.ts): سلسلة 340 (مخالفة مؤكَّدة موثَّقة) تُعرَض أولاً إن
  // تحققت شروطها لأنها الدليل الأدق على أعلى نقطة تجاوز، وإلا فسلسلة 250
  // (استمرار موحَّد أنتج التعليق الفعلي للنشاط).
