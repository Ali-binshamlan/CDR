...
export function decideFinal(input: Readonly<FinalDecisionInput>): Readonly<FinalDecision> {
  ...
  if (mode === 'PLANNING' && !isPlanningLiveDevice) {
    ...
    // لحالة "لا جهاز إطلاقاً بعد الآن".
    //
    // خطأ إعادة إنتاج مكتشَف ومُصلَح (بلاغ مباشر من المستخدم — لقطة نص:
    // "سيتم تفعيل جهاز الرصد... قبل ساعتين من موعد البدء" ظهرت رغم إلغاء
    // هامش الساعتين نهائياً في نفس الجلسة): النص كان يشير لهامش لم يعد
    // موجوداً — الجهاز يتفعّل الآن عند planned_time بالضبط، لا قبله بساعتين.
    const result: FinalDecision = {
      ...
      shortReasonAr: isFavorable
        ? 'تنبيه: هذه توقّعات طقس لوقت بدء النشاط المجدول (لم يبدأ بعد)، لا قراءة جهاز حية — الأجواء المتوقعة تصلح للنشاط. سيتم تفعيل جهاز الرصد وعرض قراءاته الحية عند بدء النشاط فعلياً.'
        : `تنبيه: هذه توقّعات طقس لوقت بدء النشاط المجدول (لم يبدأ بعد)، لا قراءة جهاز حية — ${unfavorableReasonAr ?? 'الأجواء المتوقعة لا تصلح للنشاط.'} يُرجى مراجعة توقعات الساعات القادمة قبل البدء. سيتم تفعيل جهاز الرصد وعرض قراءاته الحية عند بدء النشاط فعلياً.`,
