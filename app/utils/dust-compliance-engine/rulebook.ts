...
export function pm10ThresholdRule(
  ...
  if (pm10UgM3 > PM10_VIOLATION_STOP_UG_M3) {
    ...
    if (isConfirmed) {
      ...
      // MRQ-PM10-BLACK-PENDING-104 لم تتغيّر — قرار المستخدم يخص حصراً
      // القاعدة المؤكَّدة (PM10-VIOLATION-STOP-006) أعلاه. القراءة لم تُثبِت
      // بعد اكتمال دقيقتين استمرار، فتبقى "معلَّقة" (STOP_AFFECTED_ACTIVITY
      ...
      hits.push(
        ruleHit(
          ...
          `تعليق مؤقت (معلَّق): تركيز PM10 (${pm10UgM3} ميكروجرام/م³) تجاوز حد المخالفة (${PM10_VIOLATION_STOP_UG_M3} ميكروجرام/م³) — بانتظار اكتمال دقيقتين استمرار لتصنيفها مخالفة تنظيمية مؤكدة`,
          'أوقف النشاط فوراً احترازياً وراقب استمرار القراءة — ستُصبح مخالفة تنظيمية مؤكدة وموثَّقة (بلا إيقاف إلزامي فوري) إن استمر التجاوز دقيقتين فأكثر'
        )
      );
    }
