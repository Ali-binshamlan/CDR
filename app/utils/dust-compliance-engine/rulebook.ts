...
export function pm10ThresholdRule(
  ...
  // قرار تنظيمي مُعاد النظر فيه (طلب صريح من المستخدم — يُلغي إيقاف
  // MANDATORY_STOP الفوري عند تأكيد مخالفة 340 الموثَّق سابقاً هنا): تصعيد
  // إلى >340 لأكثر من دقيقتين كان يوقف النشاط فوراً وإلزامياً بمعزل تام عن
  // استمرارية [250,340] السابقة — الحسم التنظيمي الجديد يفصل بين "تسجيل
  // مخالفة" و"إيقاف فعلي": الإيقاف الوحيد للنشاط الآن هو RCRC-PM10-30M-
  // SUSPENSION-012 أدناه (استمرار موحَّد فوق 250 لمدة 30 دقيقة، راجع
  // computeSustainedPm10Status في dustEvaluation.ts). تأكيد مخالفة 340
  // (PM10-VIOLATION-STOP-006) أصبح توثيقاً/تنبيهاً فقط — نفس مستوى الضوابط
  // المعروض في نطاق [250,340] (ALLOW_WITH_CONTROLS)، بلا تصعيد فعلي في
  // القيد، فقط رسالة صريحة تُعلم المستخدم بتسجيل المخالفة. `>` صراحة لا
  // `>=` عند حد المخالفة (340 بالضبط يبقى ضمن نطاق التحذير الموحَّد أدناه
    ...
    if (isConfirmed) {
      hits.push(
        ruleHit(
          ...
          'ALLOW_WITH_CONTROLS',
          `مخالفة تنظيمية مؤكدة ومسجَّلة: تركيز PM10 (${pm10UgM3} ميكروجرام/م³) تجاوز حد المخالفة (${PM10_VIOLATION_STOP_UG_M3} ميكروجرام/م³) لأكثر من دقيقتين متتاليتين — النشاط مستمر تحت الضوابط المعزَّزة، الإيقاف الفعلي مرتبط فقط باستمرار التجاوز عند ${PM10_WARNING_UG_M3} ميكروجرام/م³ فأكثر لمدة ${PM10_SUSPENSION_MINUTES} دقيقة متواصلة`,
      ...
      // MRQ-PM10-BLACK-PENDING-104 لم تتغيّر — قرار المستخدم الجديد يخص
      // حصراً القاعدة المؤكَّدة (PM10-VIOLATION-STOP-006) أعلاه. القراءة لم
      // تُثبِت بعد استمرارها >دقيقتين، فتبقى "معلَّقة" (STOP_AFFECTED_ACTIVITY
      // مؤقت، pendingConfirmation=true عبر isPendingRuleHit في engine.ts)
      // كما كانت دائماً — لا علاقة لها بتوثيق مخالفة مؤكَّدة بعد.
      hits.push(
        ruleHit(
          ...
          'أوقف النشاط فوراً احترازياً وراقب استمرار القراءة — ستُصبح مخالفة تنظيمية مؤكدة وموثَّقة (بلا إيقاف إلزامي فوري) إن استمر التجاوز أكثر من دقيقتين'
        )
      );
    }
