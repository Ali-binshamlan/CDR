...
export async function evaluateDustVisibilityWindow(
  ...
  // نفس نوع "استخدام تقدير غير ذي صلة كأنه حقيقة". الحد الفاصل (مرّ بعدة
  // تعديلات: 30 دقيقة، ثم ساعتان "جهاز الرصد يتفعّل قبل ساعتين من بداية
  // النشاط"، وأخيراً أُلغي الهامش المبكر نهائياً — "القراءات تبدأ مع بداية
  // النشاط وتقف مع نهايته"): نشاط ضمن نافذته الفعلية [planned_time,
  // planned_time+duration] بالضبط يُعامَل كـ"الآن" فيفرض قراءة الجهاز؛ نشاط
  // لم يبدأ بعد أو انتهى فعلياً يبقى بمنطقه التوقّعي الطبيعي (worst من
  // pickWorstActualHour أعلاه، مبني بـtreatAsForecast عبر allHourlyEvaluations
  // — تقدير طقس صرف، لا قراءة جهاز مقحَمة). الانتقال فوري بمجرد عبور الحد —
  // لا منطقة رمادية/تدرّج.
