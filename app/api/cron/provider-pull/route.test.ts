...
describe('GET /api/cron/provider-pull', () => {
  ...
  it('لا pull_cursor_at ولا last_pull_at إطلاقاً (اتصال جديد) → sinceMs يعتمد حد النظر للخلف الأقصى (ساعتان)', async () => {
    rpcResponses['list_active_provider_connections'] = { data: [connectionRow()], error: null };
    fetchReadingsSinceMock.mockResolvedValue(page([], Date.now()));
    ...
    const sinceMsArg = fetchReadingsSinceMock.mock.calls[0][3] as number;
    const expectedSinceMs = Date.now() - 2 * 60 * 60_000;
    expect(Math.abs(sinceMsArg - expectedSinceMs)).toBeLessThan(2000);
  });
  ...
  describe('Pagination عند بلوغ حدود المنصة — لا فقد صامت (اختبار قبول صريح)', () => {
    ...
    // "نافذة عشر دقائق وحدود ThingsBoard تقطع البيانات بصمت"): MAX_LOOKBACK_MS
    // كانت 10 دقائق — انقطاع اتصال شائع (نشر جديد/تذبذب شبكة) يتجاوزها
    // بسهولة، فتضيع القراءات قبل تلك النافذة نهائياً (لا اقتطاع صامت، لكن
    // فقدان دليل خام روتيني غير مقبول لقرار تنظيمي). رُفعت إلى ساعتين — هذا
    // الاختبار يثبت أن انقطاعاً 20 دقيقة (كان سيفقد أول 10 دقائق منه قبل هذا
    // التغيير) يُسترجَع الآن كاملاً بلا أي فقد، لأنه أقصر بكثير من السقف
    // الجديد.
    it('توقّف 20 دقيقة (لا سحب) ثم استئناف: النافذة كاملة تُجلَب عبر صفحات بلا أي فقد (أقصر من سقف الساعتين الجديد)', async () => {
      ...
      // أول sinceMs مُستلَم من الـConnector يطابق pull_cursor_at الفعلي (مطروحاً
      // منه هامش التداخل) — لا سقف الساعتين تدخّل هنا لأن 20 دقيقة أقصر منه
      // بكثير، فلا فقدان أصلاً لأي جزء من فترة الانقطاع.
      const firstSinceMs = fetchReadingsSinceMock.mock.calls[0][3] as number;
      expect(Date.now() - firstSinceMs).toBeLessThanOrEqual(20 * 60_000 + 5_000 + 2000);
      // كل الصفحات ضمن النافذة (20 دقيقة / 3 دقائق للصفحة ≈ 7 صفحات، مُقيَّدة
      // بـMAX_PAGES_PER_CONNECTION=5) تصل فعلياً بلا توقف مبكر خاطئ.
      expect(fetchReadingsSinceMock.mock.calls.length).toBeGreaterThanOrEqual(3);
      const lastUpdate = findConnectionUpdate();
      expect(lastUpdate.last_pull_success).toBe(true);
    });

    it('انقطاع أطول من سقف الساعتين (3 ساعات) → sinceMs يُطبَّق عليه MAX_LOOKBACK_MS كحد أقصى مقصود، لا تاريخ غير محدود', async () => {
      const veryStaleCursor = new Date(Date.now() - 3 * 60 * 60_000).toISOString();
      rpcResponses['list_active_provider_connections'] = {
        data: [connectionRow({ pull_cursor_at: veryStaleCursor })],
        error: null,
      };
      fetchReadingsSinceMock.mockResolvedValue(page([], Date.now()));
      ...
      const firstSinceMs = fetchReadingsSinceMock.mock.calls[0][3] as number;
      const expectedSinceMs = Date.now() - 2 * 60 * 60_000;
      expect(Math.abs(firstSinceMs - expectedSinceMs)).toBeLessThan(2000);
    });
  });
