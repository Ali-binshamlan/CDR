-- =====================================================================
-- DCR — 202608120006_device_ingest_future_timestamp_guard_and_pm10_last_at.sql
-- =====================================================================
-- خطأ مكتشَف ومُصلَح (طلب صريح من المستخدم — مراجعة كود خارجي: "وقت الحقل
-- القديم قد يصبح حديثاً بسبب حقل آخر... ومسار Pull لا يرفض timestamp
-- مستقبلياً مثل Push"):
--
-- (1) رفض timestamp مستقبلي — كان مطبَّقاً فقط في app/api/devices/ingest/
--     route.ts (مسار Push) قبل استدعاء writeDeviceReading، بهامش تسامح
--     دقيقتين (CLOCK_SKEW_TOLERANCE_MS). مسار Pull (provider-pull →
--     telemetry-worker → writeDeviceReading مباشرة، بلا مرور عبر ذلك
--     الفحص) كان يقبل أي observedAt مستقبلي بلا أي رفض. الفحص أُضيف الآن
--     أيضاً في deviceReadingWriter.ts نفسها (يغطي كلا المسارين)، وهنا في
--     RPC نفسها كدفاع ثانٍ (defense-in-depth) لأي استدعاء مستقبلي مباشر
--     للـRPC يتجاوز الطبقة التطبيقية — نفس هامش التسامح (دقيقتان) في كلا
--     الموضعين.
--
-- (2) last_pm10/last_pm10_at (project_devices) كانتا تُحدَّثان عمداً بالوقت
--     المشترك p_observed_at (أحدث حقل في الحمولة كاملة، لا وقت PM10 تحديداً
--     — راجع migration 202608040025) بحجة أنهما "سجل عام للعرض فقط، لا
--     مصدر حساب استمرار فعلي". لكن هذا يعني أن PM10 قديم + حرارة حديثة لا
--     يزال يجعل last_pm10_at يعرض وقتاً خاطئاً (وقت الحرارة، لا وقت PM10
--     الفعلي) — قد يُضلِّل أي استهلاك مستقبلي لهذا العمود تحديداً (تنبيهات
--     "الجهاز متصل"، شارات حداثة في الواجهة) حتى لو لم يؤثر على قرار
--     الاستمرار التنظيمي نفسه (ذاك محمي أصلاً عبر v_pm10_observed_at في
--     pm10_readings_history). الإصلاح: last_pm10/last_pm10_at تستخدمان
--     v_pm10_observed_at الآن (نفس المصدر المستخدَم أصلاً لـpm10_readings_
--     history)، لا p_observed_at المشترك — يطابق فعلياً معنى اسم العمود
--     ("آخر وقت PM10")، بدل وقت آخر حقل وصل في نفس الحمولة.
-- =====================================================================
...
declare
  ...
  -- نفس هامش التسامح المستخدَم في deviceReadingWriter.ts/devices/ingest/route.ts
  -- (CLOCK_SKEW_TOLERANCE_MS/FUTURE_TIMESTAMP_TOLERANCE_MS) — قيمة واحدة
  -- موثَّقة هنا يقرأها الفحص أدناه.
  v_future_tolerance interval := interval '2 minutes';
begin
  ...
  -- راجع تعليق البند (1) أعلاه — دفاع ثانٍ بعد الفحص التطبيقي في
  -- deviceReadingWriter.ts، لأي استدعاء مباشر للـRPC يتجاوز تلك الطبقة.
  if p_observed_at > p_received_at + v_future_tolerance then
    raise exception using errcode = '22023', message = 'observed_at is in the future — check device clock';
  end if;
  ...
  -- وقت PM10 المستقل (إن وُجد) قد يختلف عن p_observed_at المشترك — يخضع لنفس
  -- فحص المستقبل بمعزل عنه (راجع تعليق البند (1) أعلى الملف).
  if v_pm10_observed_at > p_received_at + v_future_tolerance then
    raise exception using errcode = '22023', message = 'pm10 observed_at is in the future — check device clock';
  end if;
  ...
  set
    ...
    -- خطأ مكتشَف ومُصلَح (البند (2) أعلاه): last_pm10/last_pm10_at تستخدمان
    -- الآن v_pm10_observed_at (وقت PM10 المستقل الفعلي، نفس المصدر المستخدَم
    -- في pm10_readings_history أعلاه) بدل p_observed_at المشترك — لا يعود
    -- حقل آخر حديث (حرارة/رطوبة) قادراً على جعل last_pm10_at يعرض وقتاً
    -- أحدث من قراءة PM10 الفعلية.
    last_pm10 = case
      when p_measurements ? 'pm10' and (d.last_pm10_at is null or v_pm10_observed_at >= d.last_pm10_at)
        then (p_measurements->>'pm10')::numeric else d.last_pm10 end,
    last_pm10_at = case
      when p_measurements ? 'pm10' and (d.last_pm10_at is null or v_pm10_observed_at >= d.last_pm10_at)
        then v_pm10_observed_at else d.last_pm10_at end,
