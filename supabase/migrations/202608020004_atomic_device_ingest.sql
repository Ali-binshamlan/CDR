-- =====================================================================
-- DCR — 202608020004_atomic_device_ingest.sql
-- =====================================================================
-- خطأ مكتشَف (مراجعة تصحيح خارجية): app/lib/deviceReadingWriter.ts ينفّذ
-- إدخال قراءة جهاز عبر 3 استدعاءات شبكة منفصلة تماماً (تحديث
-- project_devices، إدراج device_readings_history، إدراج
-- pm10_readings_history) بلا معاملة SQL واحدة تجمعها. فشل الاستدعاء الثاني
-- أو الثالث كان يُسجَّل بـconsole.error فقط دون أن يُرجَع كخطأ للمستدعي —
-- فتعود استجابة "success: true" رغم أن سجل الدليل التاريخي (append-only،
-- أساس إثبات الامتثال لاحقاً) لم يُكتب فعلياً. كذلك last_pm10_at/last_wind_
-- speed_at/إلخ في project_devices كانت تُحدَّث دائماً بـreceivedAt (وقت
-- وصول الخادم) لا observedAt (وقت الرصد الفعلي من الجهاز) — قراءة متأخرة
-- الوصول (شبكة بطيئة/جهاز أعاد اتصاله) كانت تُختم كأنها "حديثة الآن" في
-- اللقطة الحية المستخدَمة للقرار التشغيلي.
--
-- الإصلاح: دالة PL/pgSQL واحدة (نفس نمط persist_activity_decision_atomic
-- في 202607290007) تنفّذ التحقق + كل الكتابات الثلاث ضمن معاملة واحدة،
-- وتحدّث كل عمود last_*/_at في project_devices فقط إذا كانت هذه القراءة
-- (بحسب observed_at الفعلي، لا وقت الوصول) أحدث من القيمة المخزَّنة —
-- فحدث متأخر الوصول يُحفَظ تاريخياً في device_readings_history لكنه لا
-- يستبدل لقطة أحدث.
-- =====================================================================

-- last_pm25_at كانت غائبة عن project_devices رغم last_pm25 (لا شيء يثبت
-- حداثة قراءة PM2.5 تحديداً، بخلاف last_pm10_at الموجود لـPM10).
...
-- project_devices تفتقر last_received_at (راجع تعليق الدالة أعلاه —
-- last_reading_at العام موجود مسبقاً، last_received_at وقت وصول الخادم
-- الخام منفصل عنه للمراقبة/التأخير فقط).
alter table public.project_devices
  add column if not exists last_received_at timestamptz;

update public.project_devices
  set last_received_at = last_reading_at
  where last_received_at is null and last_reading_at is not null;

-- الدالة تُستدعى حصراً عبر service_role من app/lib/deviceReadingWriter.ts
-- (نفس نمط persist_activity_decision_atomic في 202607290007) — لا وصول
-- مباشر من anon/authenticated.
