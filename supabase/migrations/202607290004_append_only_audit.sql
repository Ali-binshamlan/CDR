...
--
-- خطأ بنيوي حرج مكتشَف ومُصلَح بأثر رجعي (حادثة إنتاج فعلية 2026-08-12،
-- مُصلَحة أصلاً في migration 202608120015 اللاحقة زمنياً — نُقل الإصلاح إلى
-- هنا، مكانه الزمني الصحيح، بعد أن لاحظ المستخدم أن أي بيئة جديدة تُبنى من
-- الصفر تفشل في نفس النقطة بالضبط: 202608120011 تُنفّذ UPDATE على
-- evidence_hash_ledger بينما هذه الدالة، لو بقيت بصيغتها الأصلية، تفشل بخطأ
-- 42703 "record NEW has no field dust_profile_id" قبل الوصول لتلك الهجرة —
-- الإصلاح اللاحق (202608120015) كان يُطبَّق بعد نقطة التوقف نفسها، عديم
-- الفائدة لأي bootstrap تسلسلي):
--
-- هذه الدالة تُطبَّق أيضاً كـtrigger على جداول أخرى لا تملك عمود
-- dust_profile_id إطلاقاً (evidence_hash_ledger لاحقاً، 202608110007 —
-- بنية مختلفة كلياً: seq/row_hash/chain_hash/إلخ). NEW هنا من نوع RECORD
-- (لا نوع صف محدَّد وقت تجميع الدالة) — PostgreSQL قد يحاول الوصول لحقل
-- dust_profile_id على NEW *قبل* أن يُقيَّم شرط TG_TABLE_NAME بالكامل بأمان
-- ضمن عبارة AND واحدة مركَّبة (تقييم AND متعدد الشروط على RECORD لا يضمن
-- دائماً short-circuit يمنع الوصول للحقل غير الموجود)، فيفشل بخطأ 42703 عند
-- أي UPDATE حقيقي على جدول لا يملك هذا العمود — بصرف النظر عن قيمة
-- TG_TABLE_NAME الفعلية. الإصلاح: فحص TG_TABLE_NAME في IF منفصل أولاً
-- (يُقيَّم ويُرفَض بالكامل قبل أي وصول لـNEW.dust_profile_id) بدل شرط AND
-- واحد مركَّب — لا وصول لحقل dust_profile_id إطلاقاً إلا بعد التأكد أن
-- TG_TABLE_NAME يطابق أحد الجدولين اللذين يملكانه فعلاً. نفس المنطق
-- والسلوك تماماً لكل الجداول الأخرى — لا تغيير في أي دلالة، فقط ترتيب
-- تقييم آمن. استثناء إضافي خاص بـevidence_hash_ledger.coverage_started_at
-- يُضاف لاحقاً في migration 202608120015 (بعد إنشاء ذلك الجدول والعمود).
