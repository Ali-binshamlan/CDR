-- =====================================================================
-- DCR — 202608160004_persist_decision_return_final_decision_id.sql
-- =====================================================================
-- خطأ معماري مكتشَف ومُصلَح (مراجعة كود خارجي — المشكلة 4، P1: "React ما
-- زالت تحسب DVI وAEI وتعرض الحفظ كتقييم رسمي"): AddActivityModal/index.tsx
-- كان يحسب DVI/AEI في المتصفح ويعرض "تم التقييم والحفظ بنجاح" فوراً، رغم
-- أن الخادم يرفض قيم aei_score/aei_status القادمة من العميل (لا يثق بها)
-- ولا يُصدِر final_decisions ضمن نفس طلب الحفظ — القرار الرسمي قد لا يكون
-- صدر بعد فعلياً وقت عرض رسالة النجاح. الإصلاح الكامل (commit مرافق):
-- الواجهة ترسل بيانات خام فقط، ثم تستدعي POST /evaluate الموجود أصلاً بعد
-- الحفظ، وتتحقق من وجود صف final_decisions حقيقي لهذا النشاط تحديداً قبل
-- عرض "صدرت نتيجة القرار الرسمي" — لا مجرد نجاح استدعاء evaluate عاماً.
--
-- هذا يتطلب معرفة final_decisions.id الفعلي المُدرَج لكل نشاط على حدة —
-- persist_activity_decision_atomic كانت تحسب v_final_decision_id داخلياً
-- (returning id into، migration 202608160002) لكن لا تُعيده في returns
-- table (بوليانات فقط) — التوسعة هنا إضافية بحتة (عمود جديد في النتيجة)،
-- بلا أي تغيير على معاملات الدخول أو منطق الكتابة نفسه — provider-pull/
-- devices-ingest (المستدعيان الآخران عبر evaluateProject/persistActivityDecisionsAtomic)
-- لا يحتاجان هذا الحقل الجديد ولا يتأثران به إطلاقاً.
-- =====================================================================
...
returns table (
  ...
  -- جديد (202608160004): معرّف صف final_decisions المُدرَج فعلياً هذه
  -- الدورة، إن أُدرِج (final_decision_persisted=true) — null غير ذلك
  -- (تخطّي v_skip_final، أو p_final_decision=null أصلاً). يسمح للمستدعي
  -- (evaluateProject → POST /evaluate) بمعرفة القرار الرسمي المحدَّد لكل
  -- نشاط بدقة، لا مجرد "نجح التقييم إجمالاً على مستوى المشروع".
