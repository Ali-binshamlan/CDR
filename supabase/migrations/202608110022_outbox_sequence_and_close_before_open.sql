-- =====================================================================
-- DCR — 202608110022_outbox_sequence_and_close_before_open.sql
-- =====================================================================
-- خطأ مكتشَف ومُصلَح (مراجعة كود خارجي — "الانتقال بين أنواع التنبيه لا
-- يغلق النوع السابق"): persist_activity_decision_atomic (202608110021)
-- كانت تُدرج نية CLOSE للنوع السابق (v_previous_required_alert_kind) فقط
-- في فرع else — أي فقط حين v_required_alert_kind الجديد = null (النشاط
-- أصبح COMPLIANT/بلا استوجاب تنبيه إطلاقاً). انتقال فعلي بين نوعين مختلفين
-- كليهما غير null (مثال: SAFETY_BREACH → COMPLIANCE_RESTRICTION، نشاط كان
-- إيقافاً إلزامياً ثم أصبح تقييداً فقط دون إيقاف) كان يُدرِج نية OPEN
-- للنوع الجديد فقط — النوع السابق (SAFETY_BREACH) يبقى صفه في alerts مفتوحاً
-- للأبد، فيرى المستخدم تنبيهين متناقضين نشطين معاً لنفس النشاط.
--
-- مشكلة ثانية مرتبطة: claim_alert_outbox_batch (202608040026) كانت تُرتِّب
-- `order by id` — id عمود uuid عشوائي (gen_random_uuid() افتراضي)، فحتى لو
-- أُدرجت نيتا CLOSE وOPEN معاً بالترتيب الصحيح داخل نفس المعاملة، لا ضمان
-- أن العامل يُعالِج CLOSE قبل OPEN عند المطالبة بدفعة تحويهما معاً — قد
-- يُعالَج OPEN أولاً (فيُنشئ تنبيهاً جديداً بينما القديم لا يزال مفتوحاً
-- للحظة، أو العكس زمنياً حسب ترتيب uuid العشوائي).
--
-- الحل (الحد الأدنى المطلوب صراحةً، بعد مراجعة بديل Reconciler الذري
-- الأشمل وتفضيل النطاق الأضيق): sequence_no bigint متزايد صرامةً على
-- decision_alert_outbox، claim_alert_outbox_batch تُرتِّب به بدل id،
-- وpersist_activity_decision_atomic تُدرِج نية CLOSE للنوع السابق (إن وُجد
-- ويختلف عن الجديد) *قبل* نية OPEN للنوع الجديد (إن وُجد) — لا فرع else
-- حصري بعد اليوم، بل شرطان مستقلان بالترتيب. sequence_no المتزايد يضمن أن
-- CLOSE (يُدرَج أولاً زمنياً داخل نفس المعاملة) يحمل قيمة أصغر من OPEN
-- التالية له مباشرة، فيُعالِجهما claim_alert_outbox_batch بنفس الترتيب.
-- =====================================================================

alter table public.decision_alert_outbox
  add column if not exists sequence_no bigint generated always as identity;

comment on column public.decision_alert_outbox.sequence_no is
  'ترتيب إدراج متزايد صرامةً — claim_alert_outbox_batch تُرتِّب به بدل id (uuid عشوائي لا يضمن ترتيب المعالجة). يضمن معالجة CLOSE قبل OPEN عند انتقال بين نوعي تنبيه مختلفين لنفس القرار (كلاهما يُدرَجان بنفس المعاملة، CLOSE أولاً).';

create index if not exists idx_decision_alert_outbox_sequence
  on public.decision_alert_outbox (sequence_no);

-- ---------------------------------------------------------------------
-- claim_alert_outbox_batch — نفس المنطق تماماً (202608040026)، فقط
-- order by sequence_no بدل order by id.
-- ---------------------------------------------------------------------
...
as $$
  ...
  candidates as (
    select o.id
    from public.decision_alert_outbox o
    where o.id in (select id from union_ids)
...
-- ---------------------------------------------------------------------
-- persist_activity_decision_atomic — نفس التوقيع (18 معاملاً)، بلا تغيير
-- على أي شيء آخر عدا قسم إدراج نوايا الـOutbox: CLOSE للنوع السابق (إن
-- اختلف عن الجديد) يُدرَج أولاً، ثم OPEN للنوع الجديد (إن وُجد) — مستقلان،
-- لا else حصري بينهما.
-- ---------------------------------------------------------------------
  ...
  if p_final_decision is not null then
    ...
    if not v_skip_final then
      ...
      if v_mode = 'LIVE_OPERATIONAL' then
        -- خطأ مكتشَف ومُصلَح (مراجعة كود خارجي — "الانتقال بين أنواع
        -- التنبيه لا يغلق النوع السابق"): شرطان مستقلان الآن بدل else حصري
        -- — CLOSE للنوع السابق يُدرَج أولاً (إن اختلف فعلياً عن الجديد، بما
        -- في ذلك حالة "أصبح null" السابقة)، ثم OPEN للنوع الجديد (إن وُجد)
        -- — بصرف النظر عمّا إذا كان الآخر موجوداً. sequence_no المتزايد
        -- (migration هذه، عمود جديد) يضمن أن صف CLOSE (يُدرَج فعلياً أولاً
        -- هنا) يحمل قيمة أصغر من صف OPEN التالي مباشرة، فيُعالِجهما العامل
        -- (claim_alert_outbox_batch يُرتِّب بـsequence_no الآن) بنفس الترتيب
        -- — لا احتمال معالجة OPEN قبل CLOSE للانتقال نفسه بعد اليوم.
