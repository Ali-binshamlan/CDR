-- =====================================================================
-- DCR — 202608100001_final_decisions_state_change_skip.sql
-- =====================================================================
-- المرحلة 6 من خطة إعادة تصميم مسار استقبال Telemetry (طُبِّقت بموافقة
-- صريحة 2026-08-09/10) — State-Change Processing لـfinal_decisions.
--
-- الفجوة المُصلَحة: current_dust_decisions/current_dust_compliance_decisions
-- محميان بالفعل من الكتابة الزائدة عبر shouldSkipPersist (جانب TypeScript،
-- dustEvaluation.ts:1030-1039 — يتخطى فقط لو نفس القرار خلال 5 دقائق)، لكن
-- final_decisions (سجل القرار الظاهر للوحة، append-only) لم يكن يملك حارساً
-- مماثلاً — كان يُدرَج صف جديد في كل مرة يوجد فيها p_final_decision، حتى لو
-- كانت الحالة التشغيلية مطابقة تماماً للصف السابق (201→202→203→204 بلا
-- تغيّر حالة فعلي كانت تُنتج 4 صفوف final_decisions منفصلة).
--
-- الإصلاح: حارس تخطٍ جديد قبل INSERT في final_decisions مباشرة — يتخطى
-- الكتابة فقط إذا اجتمعت الشروط الثلاثة معاً:
--   1) operational_decision نفسه كسابقه (بحسب آخر صف LIVE_OPERATIONAL).
--   2) level نفسه كسابقه.
--   3) pending_confirmation نفسه كسابقه.
--   4) الصف السابق حديث (ضمن نافذة 5 دقائق — نفس MIN_MINUTES_BETWEEN_
--      UNCHANGED_EVALUATIONS المستخدَمة في shouldSkipPersist، dustEvaluation.ts:1028).
--
-- حارس إلزامي إضافي (لا يجوز إغفاله — القسم 8 من خطة إعادة التصميم):
-- انتقال حالة التعليق/الاستئناف (current_dust_compliance_decisions.
-- pending_resume_since) يُبطل التخطي فوراً بصرف النظر عن تطابق الحقول
-- الثلاثة أعلاه — ساعة التعليق نفسها جزء مما يحتاج المدقق التنظيمي رؤيته،
-- حتى لو ظل operational_decision/level يبدوان كما هما ظاهرياً. يُقارَن
-- p_compliance_pending_resume_since (القيمة الجديدة الممرَّرة لهذا الاستدعاء)
-- بالقيمة المخزَّنة حالياً في current_dust_compliance_decisions *قبل* أي
-- تحديث عليها في هذه المعاملة (تُقرَأ في الجزء 2 أعلاه بالفعل قبل وصولنا هنا).
--
-- لا تغيير في أي عتبة/منطق قرار (PM10 250/340، دقيقتان/30 دقيقة، نطاقات
-- الرياح) — هذا حارس "متى نكتب سجلاً" فقط، لا "ماذا نقرر".
-- =====================================================================
...
declare
  ...
  -- جديد — المرحلة 6: حالة الصف السابق الكاملة (لا operational_decision
  -- وحده كما كان سابقاً) + حالة التعليق الحالية المخزَّنة، لبناء حارس
  -- التخطي بدقة.
...
begin
  ...
  if p_compliance_result is not null then
    -- جديد — المرحلة 6: قراءة pending_resume_since الحالية *قبل* تحديثها
    -- أدناه، لاستخدامها لاحقاً في حارس final_decions (القسم 3). تُقرأ هنا
    -- تحديداً (لا تعتمد على existing lookup في TypeScript) لأن هذه الدالة
    -- قد تُستدعى مباشرة أو عبر مسارات أخرى غير persistActivityDecisionsAtomic
    -- مستقبلاً — الحارس يجب أن يكون صحيحاً بمعزل عن المُستدعي.
  ...
  -- ===================================================================
  -- 3) final_decisions — append-only. المرحلة 6: حارس State-Change جديد
  --    قبل INSERT مباشرة (لا تغيير في بنية الصف نفسه ولا في أي منطق قرار).
  -- ===================================================================
  if p_final_decision is not null then
    ...
    -- حارس التخطي: فقط لأنشطة LIVE_OPERATIONAL (نفس نطاق منطق outbox
    -- أدناه — أنشطة PLANNING لا تحمل نفس مفهوم "استمرارية حالة تشغيلية"
    -- بين تقييمات متتالية، فلا تُقاس بنفس المعيار).
    if v_mode = 'LIVE_OPERATIONAL'
      ...
      -- الحارس الإلزامي: أي تغيّر في ساعة التعليق/الاستئناف يُبطل التخطي
      -- فوراً، بصرف النظر عن تطابق الحقول أعلاه.
