-- =====================================================================
-- DCR — 202608091002_function_scoped_lock_timeout_and_activity_serialization.sql
-- =====================================================================
-- حادثة إنتاج فعلية (2026-08-09، مباشرة بعد نشر 202608091001): استبدال
-- الأقفال الاستشارية بأقفال صف حقيقية (SELECT/INSERT ... FOR UPDATE) نقل
-- التزاحم من wait_event=advisory إلى wait_event=tuple — لم يُصلحه. الدليل
-- (pg_blocking_pids أثناء الحادثة): ~10 اتصالات authenticator متزاحمة
-- معاً على نفس صف current_dust_decisions لنفس (project_id,
-- activity_group_id) — تزامن حقيقي لعدة مصادر تكتب لنفس النشاط بنفس
-- اللحظة تقريباً (provider-pull كل دقيقة + scheduler-worker يعالج مهام
-- retry + push مباشر محتمل من devices/ingest)، لا مشكلة في نوع القفل نفسه.
--
-- السبب الجذري الفعلي المكتشَف بمراجعة الكود (app/api/cron/provider-pull/
-- route.ts، withTimeout): Promise.race(evaluateProject(...), timeout)
-- يُنهي انتظار Node.js للنتيجة بعد EVAL_TIMEOUT_MS (20 ثانية)، لكنه لا
-- يُلغي الاستعلام الفعلي في Postgres — الاتصال يبقى مفتوحاً منتظراً القفل
-- إلى ما لا نهاية على مستوى الخادم، رغم أن طبقة التطبيق "نسيته" بالفعل.
-- مع provider-pull كل دقيقة، كل دورة فاشلة تراكم اتصالاً معلَّقاً إضافياً
-- فوق سابقيه. الأسوأ: كل فشل يستدعي enqueueEvaluationRetryJob (بلا أي
-- تجميع فعلي رغم اسم dedupe_key — كل استدعاء يبني مفتاحاً فريداً عبر
-- crypto.randomUUID())، فيضيف مهمة جديدة يعالجها scheduler-worker لاحقاً
-- كمصدر تزامن إضافي على نفس الصف — حلقة تغذية راجعة سلبية تتفاقم بمرور
-- الوقت لا تتعافى من نفسها.
--
-- الإصلاح هنا (مستوى قاعدة البيانات فقط — إصلاح مسار الكود في
-- provider-pull/route.ts وevaluateProject.ts منفصل، راجع الملفات المعدَّلة
-- بنفس هذه الدفعة): SET LOCAL lock_timeout داخل persist_activity_decision_
-- atomic نفسها (لا فقط على مستوى الـrole) يضمن أن أي محاولة قفل صف عالقة
-- تفشل فعلياً على الخادم خلال ثوانٍ معدودة — بصرف النظر عمّا يفعله العميل
-- (Node.js) بعد ذلك. هذا لا يمنع التزامن نفسه (التحديث كل دقيقة ضروري
-- وليس هو المشكلة)، لكنه يضمن ألا يبقى أي اتصال عالقاً لأبد، فتتحرر
-- الاتصالات المحجوزة بسرعة كافية لمنع استنزاف connection pool الكامل مهما
-- بلغ التزاحم اللحظي.
-- =====================================================================
...
begin
  -- SET LOCAL (لا ALTER ROLE) — يسري فقط داخل هذه المعاملة، يُلغى تلقائياً
  -- عند COMMIT/ROLLBACK، لا يحتاج إعادة اتصال ليأخذ مفعوله (خلافاً لإعداد
  -- على مستوى الـrole الذي لا يُطبَّق إلا على جلسات جديدة — نفس القصور
  -- المكتشَف في 202608081001 مع اتصالات PostgREST المجمَّعة القديمة).
  -- 3 ثوانٍ (أقصر من الإعداد العام 5 ثوانٍ على الـrole عمداً) — هذه الدالة
  -- تُستدعى بمعدل تكرار أعلى بكثير (كل دقيقة من عدة مصادر) من أي دالة
  -- أخرى، فالانتظار الطويل هنا تحديداً هو الأخطر على استنزاف connection
  -- pool؛ فشل سريع + إعادة محاولة لاحقة أفضل من انتظار طويل يحجز اتصالاً.
