-- =====================================================================
-- DCR — 202608200003_persist_decision_guards_archived_activity.sql
-- =====================================================================
-- خطأ سباق مكتشَف (مراجعة كود — "أرشفة نشاط أثناء دورة تقييم متزامنة"):
-- DELETE /api/activities (أرشفة نشاط واحد، app/api/activities/route.ts)
-- يقرأ صف project_dust_profiles → يحذف current_dust_decisions/
-- current_dust_compliance_decisions → يضبط archived_at، عبر ثلاث استدعاءات
-- Supabase منفصلة تماماً (لا RPC ذرّي واحد). persist_activity_decision_atomic
-- (هذه الدالة) كانت تتحقق فقط من عدم أرشفة *المشروع* بالكامل
-- (v_project_archived_at، راجع migration السابقة) — لا من أرشفة *النشاط*
-- نفسه (project_dust_profiles.archived_at).
--
-- السيناريو: دورة تقييم (evaluateProject.ts) بدأت قبل طلب الأرشفة وتجاوزت
-- فحصها الوحيد لـarchived_at IS NULL (في بداية الدورة فقط، قبل عدة awaits
-- لطلبات طقس/امتثال) — إن اكتمل طلب الأرشفة (حذف + ضبط archived_at) قبل
-- أن تصل تلك الدورة لاستدعاء persist_activity_decision_atomic، يعيد
-- الاستدعاء المتأخر إدراج صف current_dust_decisions/current_dust_compliance_decisions
-- حي وصف final_decisions جديد لنشاط أُرشِف للتو — ينتهك ثابتاً معمارياً
-- (لا حالة قرار حية لنشاط مؤرشَف) مطبَّق في كل مكان آخر عبر فحوصات
-- archived_at IS NULL.
--
-- الإصلاح: نفس نمط PROJECT_ARCHIVED بالضبط، على مستوى النشاط — الفحص
-- الموجود أصلاً (تطابق activity_id/project_id/activity_group_id) يُضاف له
-- شرط p.archived_at is null، فيفشل الاستدعاء برمز ACTIVITY_ARCHIVED صريح
-- بدل إدراج صف حي زائف. يحدث تحت نفس advisory lock (project_id) المُتخذ
-- أعلاه بالفعل (select ... for update على public.projects) — لا حاجة لقفل
-- إضافي، فالتحقق من الأرشفة يقع داخل نفس الجزء الحرج المحمي أصلاً.
-- =====================================================================
...
begin
  ...
  -- خطأ سباق مُصلَح (راجع تعليق الملف الكامل أعلاه): p.archived_at is null
  -- أُضيفت لهذا الفحص — نشاط أُرشِف عبر DELETE /api/activities بين بداية
  -- دورة تقييم متزامنة ووصولها لهذه النقطة لا يعود يمرّ من هنا بصمت.
  perform 1
  ...
  if not found then
    -- تمييز سبب الفشل: نشاط أُرشِف فعلياً (موجود لكن archived_at ليس null)
    -- مقابل عدم تطابق activity_id/project_id/activity_group_id الأصلي —
    -- رمز خطأ منفصل (P0001 مثل PROJECT_ARCHIVED) للحالة الأولى، بدل رمي
    -- 23503 العام نفسه لكلتا الحالتين، حتى يميّزهما المستدعي في dustEvaluation.ts.
    perform 1
    from public.project_dust_profiles p
    where p.id = p_activity_id::uuid
      and p.project_id = p_project_id
    ...
    if found then
      raise exception using errcode = 'P0001', message = 'ACTIVITY_ARCHIVED';
    end if;
