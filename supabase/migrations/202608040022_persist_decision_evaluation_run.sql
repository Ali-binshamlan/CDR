-- =====================================================================
-- DCR — 202608040022_persist_decision_evaluation_run.sql
-- =====================================================================
-- يُضيف p_evaluation_run_id وp_input_snapshot_hash إلى persist_activity_
-- decision_atomic (القسم 20، بند 7) — يُخزَّنان في final_decisions
-- (202608040021)، ويُنسَخ evaluation_run_id أيضاً إلى decision_alert_outbox
-- عند إنشاء نية تنبيه، بحيث القرار والتنبيه المرتبط به يحملان نفس
-- evaluation_run_id (بالإضافة إلى final_decision_id الموجود مسبقاً).
-- كلا المعاملين الجديدين اختياريان (nullable) — استدعاءات قديمة/اختبارات
-- لا تُمرِّرهما تبقى تعمل (توافقي، فشل آمن نحو null بدل كسر أي مستدعٍ حالي).
-- =====================================================================
