-- =====================================================================
-- DCR — 202608030005_fix_first_insert_cas_race.sql
-- =====================================================================
-- خطأ حرج مكتشَف ومُصلَح (مراجعة كود خبير خارجي — "أول إدراج متزامن ما زال
-- يستطيع تسجيل نجاح زائف وقرار نهائي إضافي"): في persist_activity_decision_atomic
-- (راجع 202607290007_atomic_decision_persist.sql)، فرع "لا صف سابق"
-- (p_dvi_expected_updated_at IS NULL أو p_compliance_expected_updated_at
-- IS NULL) كان يستخدم "insert ... on conflict do nothing"، فإن خسر السباق
-- (منافس متزامن سبقه لإدراج نفس activity_group_id)، يحاول تحديثاً احتياطياً
-- بشرط "where updated_at is null" — لكن هذا الشرط لا يتحقق أبداً عملياً:
-- كل مسار إدراج (الفائز والفرع الاحتياطي نفسه) يضبط updated_at = now() فوراً،
-- فلا صف يبقى بـupdated_at IS NULL بعد إدراجه الأول أياً كان الفائز. النتيجة:
-- الطلب الخاسر لا يُحدِّث current_dust_decisions/current_dust_compliance_
-- decisions بقيمته إطلاقاً (تبقى بقيمة الفائز)، لكن الدالة كانت تستمر بصمت
-- وتضبط v_dvi_persisted/v_compliance_persisted := true رغم ذلك، ثم تُدرِج
-- final_decisions (السطر يعتمد على v_final_persisted فقط، لا على نجاح فعلي
-- لتحديث current_*) — فيُنتَج قرار نهائي إضافي مبني على حالة current_*
-- لم تُكتب فعلياً بقيمة هذا الطلب، بلا أي إشارة فشل ترجع للتطبيق.
--
-- الإصلاح: الشرط الاحتياطي غير القابل للتحقق يُستبدَل بفحص ROW_COUNT صريح
-- بعد "on conflict do nothing" مباشرة (لا تحديث احتياطي وهمي بشرط لن يتحقق
-- أبداً) — إن لم يُدرَج شيء (ROW_COUNT = 0، يعني منافس سبقنا فعلاً)، تفشل
-- الدالة بنفس كود 40001 (serialization_failure) المستخدَم أصلاً لتعارض CAS
-- العادي في الفرع الآخر (existing.updated_at تغيّر) — التطبيق (راجع
-- persistActivityDecisionsAtomic في app/lib/dustEvaluation.ts) يتعامل مع
-- 40001 كتعارض تزامن طبيعي قابل لإعادة المحاولة في الدورة التالية أصلاً،
-- لا كفشل حقيقي؛ لا تغيير مطلوب في التطبيق لهذا الإصلاح.
-- =====================================================================
...
begin
  ...
  if p_dvi_result is not null then
    ...
    if p_dvi_expected_updated_at is not null then
      ...
      get diagnostics v_affected_rows = row_count;
      if v_affected_rows <> 1 then
        -- منافس متزامن سبقنا لإدراج أول صف لنفس activity_group_id — لا يوجد
        -- تحديث احتياطي آمن ممكن هنا (صف المنافس أُدرج بـupdated_at = now()
        -- فوراً، فلا شرط WHERE يميّزه بأمان عن أي تحديث لاحق شرعي آخر). نفشل
        -- بنفس كود CAS العادي بدل الاستمرار الصامت بنجاح زائف.
        raise exception using errcode = '40001',
  ...
  if p_compliance_result is not null then
    ...
    if p_compliance_expected_updated_at is not null then
      ...
      get diagnostics v_affected_rows = row_count;
      if v_affected_rows <> 1 then
        -- نفس منطق current_dust_decisions أعلاه — فشل صريح بدل نجاح زائف.
        raise exception using errcode = '40001',
...
-- نفس امتيازات الدالة الأصلية بالضبط (create or replace يحافظ عليها فعلياً،
-- لا حاجة لتكرار revoke/grant) — يُبقيها service_role فقط، بلا تغيير.
