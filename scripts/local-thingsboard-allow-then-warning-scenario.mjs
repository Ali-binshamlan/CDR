// سيناريو اختبار مبسّط لنفس جهاز ThingsBoard: يبدأ بضمان عودة النشاط لحالة
// "مسموح" (ALLOW) فعلياً — PM10 تحت 200 لمدة كافية — ثم ينتقل لمرحلة
// تحذير/ضوابط إلزامية (251-339، دون حد المخالفة 340).
//
// المراحل:
//   1) ALLOW    — PM10 بين 50-150 لمدة (افتراضياً 7 دقائق).
//   2) WARNING  — PM10 بين 251-339 لمدة (افتراضياً 7 دقائق).
//
// الرؤية ثابتة طوال السيناريو — لا تتداخل مع اختبار PM10.
// سكربت اختباري محلي فقط — لا علاقة له بكود التطبيق، لا يُشغَّل بالإنتاج.
//
// الاستخدام:
//   THINGSBOARD_DEVICE_TOKEN=<device-access-token> node scripts/local-thingsboard-allow-then-warning-scenario.mjs
// اختياري: THINGSBOARD_BASE_URL (افتراضي https://thingsboard.cloud)،
// SCENARIO_SEND_INTERVAL_MS (افتراضي 60000 = دقيقة واحدة)، ALLOW_MINUTES
// (افتراضي 7)، WARNING_MINUTES (افتراضي 7).
...
const STAGES = [
  {
    name: 'ALLOW (سماح نظيف 50-150)',
    minutes: Number(process.env.ALLOW_MINUTES) || 7,
    pm10Min: 50,
    pm10Max: 150,
  },
  {
    name: 'WARNING (تحذير/ضوابط إلزامية 251-339، دون حد المخالفة 340)',
    minutes: Number(process.env.WARNING_MINUTES) || 7,
    pm10Min: 251,
    pm10Max: 339,
  },
];
...
async function runScenario() {
  ...
  console.log(`بدء سيناريو (مسموح ثم تحذير) → ${BASE_URL}/api/v1/${DEVICE_TOKEN}/telemetry`);
